...
def get_page_as_json(session: Session, base_url: str, page_name: str) -> list[str]:
    ...
    raise URLError(f'Could not find wikitext for page {page_name}. Response: {json_data}')
...
def edit_field(txtwiki: str, kbank_field_name: str, kbank_field_val: str) -> str:
    """ Converts provided string to a Wikicode object and checks if field is present and populated, then replaces val """   
    wikicode = mwp.parse(txtwiki)
    template = wikicode.filter_templates()[0]
    node = template.get(kbank_field_name, None)
    if node and len(list(filter(lambda a: not a.isspace(), filter(str.isprintable, node.value)))) != 0:
        template.add(kbank_field_name, kbank_field_val)
        wikicode.replace(wikicode.filter_templates()[0], template)
        return str(wikicode)
    else:
        raise AssertionError(f'Veld "{kbank_field_name}" is ontbreekt of is leeg.')
...
def main():
    ...
    kbank_field_name = input('Welk veld moet worden gewijzigd? (Zoals op de Kennisbank weergegeven, bv. "Gerelateerd aan gezicht" zonder quotes.): ')
    ...
    for index, row in df.iterrows():
        try:
            ...
            new_page_content = edit_field(page_data, kbank_field_name, row[xls_field_name])
